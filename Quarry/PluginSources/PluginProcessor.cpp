...
QuarryAudioProcessor::QuarryAudioProcessor()
    : mAPVTS(*this, nullptr, NnId::ParametersId, ParameterHelpers::createParameterLayout())
{
    // Enable logging or not
#if 0
    mLogger.reset(FileLogger::createDefaultAppLogger("/tmp/Quarry", "log.txt", "YO! \n"));
...
void QuarryAudioProcessor::prepareToPlay(double sampleRate, int samplesPerBlock)
{
...
void QuarryAudioProcessor::processBlock(AudioBuffer<float>& buffer, MidiBuffer& midiMessages)
{
...
AudioProcessorEditor* QuarryAudioProcessor::createEditor()
{
    return new QuarryEditor(*this);
}

void QuarryAudioProcessor::getStateInformation(MemoryBlock& destData)
{
    auto full_state_tree = ValueTree(NnId::FullStateId);

    full_state_tree.setProperty(NnId::QuarryVersionId, ProjectInfo::versionString, nullptr);
...
void QuarryAudioProcessor::setStateInformation(const void* data, int sizeInBytes)
{
    ...
    if (xmlState != nullptr) {
        ...
        if (full_state_tree.isValid() && full_state_tree.hasType(NnId::FullStateId)) {
            auto new_value_tree = full_state_tree.getChildWithName(NnId::QuarryStateId);
...
SourceAudioManager* QuarryAudioProcessor::getSourceAudioManager() const
{
    return mSourceAudioManager.get();
}

AudioInputManager* QuarryAudioProcessor::getAudioInputManager() const
{
    return mAudioInputManager.get();
}

Player* QuarryAudioProcessor::getPlayer() const
{
    return mPlayer.get();
}

TranscriptionManager* QuarryAudioProcessor::getTranscriptionManager() const
{
    return mTranscriptionManager.get();
}

std::array<RangedAudioParameter*, ParameterHelpers::TotalNumParams>& QuarryAudioProcessor::getParams()
{
    return mParams;
}

float QuarryAudioProcessor::getParameterValue(ParameterHelpers::ParamIdEnum inParamId) const
{
    return ParameterHelpers::getUnmappedParamValue(mParams[inParamId]);
}

QuarryMainView* QuarryAudioProcessor::getQuarryMainView() const
{
    auto* editor = dynamic_cast<QuarryEditor*>(getActiveEditor());
...
AudioProcessorValueTreeState& QuarryAudioProcessor::getAPVTS()
{
    return mAPVTS;
}

ValueTree& QuarryAudioProcessor::getValueTree()
{
    return mValueTree;
}

void QuarryAudioProcessor::addListenerToStateValueTree(ValueTree::Listener* inListener)
{
    mValueTree.addListener(inListener);
}

void QuarryAudioProcessor::removeListenerFromStateValueTree(ValueTree::Listener* inListener)
{
    mValueTree.removeListener(inListener);
}

ValueTree QuarryAudioProcessor::_createDefaultValueTree()
{
    ValueTree default_value_tree(NnId::QuarryStateId);
...
void QuarryAudioProcessor::_updateValueTree(const ValueTree& inNewState)
{
    jassert(inNewState.getType() == NnId::QuarryStateId);
...
{
    return new QuarryAudioProcessor();
}
