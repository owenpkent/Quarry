...
# Typically cmake-build-release/Quarry_artefacts/Release or build/Quarry_artefacts/Release
PLUG_DIR=${1:-}
...
for dir in "$PLUG_DIR"/{Standalone/Quarry.app,AU/Quarry.component,VST3/Quarry.vst3}; do
	if ! test -d "$dir"; then
...
NOTARY_PROFILE="${NOTARY_PROFILE:-Quarry}"
...
chmod +x "$PLUG_DIR"/{Standalone/Quarry.app,AU/Quarry.component,VST3/Quarry.vst3}/Contents/MacOS/Quarry

echo "Signing Standalone, AU and VST3"
codesign --remove-signature "$PLUG_DIR"/{Standalone/Quarry.app,AU/Quarry.component,VST3/Quarry.vst3} || true
codesign --entitlements entitlements.plist --options=runtime -s "$signingID" "$PLUG_DIR"/{Standalone/Quarry.app,AU/Quarry.component,VST3/Quarry.vst3}

# Check signature
printf "\nVerifying signature app\n"
codesign -dv --verbose=4 "$PLUG_DIR"/Standalone/Quarry.app

printf "\nVerifying signature VST3\n"
codesign -dv --verbose=4 "$PLUG_DIR"/VST3/Quarry.vst3

printf "\nVerifying signature AU\n"
codesign -dv --verbose=4 "$PLUG_DIR"/AU/Quarry.component

# Build installer
echo "Building installer"
packagesbuild -F "$PLUG_DIR" Installers/Mac/Quarry.pkgproj
mv Installers/Mac/build/Quarry.pkg Installers/Mac/build/Quarry_unsigned.pkg

# Sign installer
echo "Signing installer"
product_sign_ID=$(security find-identity -v -p basic | grep "Developer ID Installer" | head -1 | cut -d'"' -f2)
productsign --sign "$product_sign_ID" Installers/Mac/build/Quarry_unsigned.pkg Installers/Mac/build/Quarry.pkg
rm Installers/Mac/build/Quarry_unsigned.pkg

# Notarize the pkg and staple it
echo "Notarize and staple installer"
xcrun notarytool submit --keychain-profile "$NOTARY_PROFILE" --wait Installers/Mac/build/Quarry.pkg
xcrun stapler staple Installers/Mac/build/Quarry.pkg
