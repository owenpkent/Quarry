...
/**
 * The AUDIO INPUT panel: drops down from the toolbar, picks the audio input NeuralNoteVideo
 * records from, and shows its level so it's obvious whether signal is arriving before hitting
 * record. Hidden by default; NeuralNoteMainView owns it and toggles it.
 */
...
public:
     ...
     */
    AudioInputView(NeuralNoteAudioProcessor& inProcessor, std::function<void()> inOnRecordClicked);
